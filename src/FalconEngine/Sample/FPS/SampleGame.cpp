...
SampleGame::UpdateFrame(GameEngineGraphics *graphics, GameEngineInput *input, double elasped)
{
    ...
    //sDebugRenderer->AddSphere(mCamera->GetPosition(), 5.0f, ColorPalette::Red, 0.0f, false);
    // sDebugRenderer->AddAabb(mPointLight1.get(), Transparent(ColorPalette::Yellow, 1.0f), 0.0f, true);
    // sDebugRenderer->AddAabb(mPointLight2.get(), Transparent(ColorPalette::Green, 1.0f), 0.0f, true);
